...
for (gen in 1:11) {
  ...
  gen_data <- data.frame(
    generation = gen,
    individual_id = names(cohorts),
    cohort = as.character(cohorts),
    bv = as.vector(bv),
    stringsAsFactors = FALSE
  )

  bv_by_gen <- rbind(bv_by_gen, gen_data)
}

# View structure
head(bv_by_gen)

# Calculate mean BV per generation using aggregate
mean_bv <- aggregate(bv ~ generation, data = bv_by_gen, FUN = mean)
mean_bv

# Or using dplyr (if installed)
# library(dplyr)
# bv_summary <- bv_by_gen %>%
#   group_by(generation) %>%
#   summarise(
#     mean_bv = mean(bv),
#     sd_bv = sd(bv),
...
plot(mean_bv$generation, mean_bv$bv,
     type = "b", lwd = 2, col = "dodgerblue3",
     xlab = "Generation", ylab = "Mean Breeding Value",
     main = "Genetic Gain Over Time")
grid()

# Calculate genetic gain from generation 1 to 11
genetic_gain <- mean_bv$bv[11] - mean_bv$bv[1]
cat("Total genetic gain:", round(genetic_gain, 3), "genetic SD\n")

# Histogram of all breeding values
hist(bv_by_gen$bv, col = "dodgerblue3",
     xlab = "Breeding Value", main = "Distribution of BV Across All Generations")
```
...
# Extract breeding values by generation
bv_by_gen <- data.frame()
for (gen in 1:11) {
  bv <- get.bv(population, gen = gen)
  cohorts <- get.cohorts.individual(population, gen = gen, use.id = TRUE)
  gen_data <- data.frame(generation = gen, individual_id = names(cohorts),
                         cohort = as.character(cohorts), bv = as.vector(bv))
  bv_by_gen <- rbind(bv_by_gen, gen_data)
}
mean_bv <- aggregate(bv ~ generation, data = bv_by_gen, FUN = mean)
genetic_gain <- mean_bv$bv[11] - mean_bv$bv[1]
cat("Genetic gain:", round(genetic_gain, 3), "\n")

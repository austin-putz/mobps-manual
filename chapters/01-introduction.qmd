...
Both packages are distributed as source tarballs from the MoBPS GitLab repository. The recommended approach is to **clone the repo and install from the local `.tar.gz` files** rather than from GitHub, which ensures you get the exact tested versions.

#### Get the Tarballs

**Option A — Clone the MoBPS software repo (recommended):**

```bash
git clone git@git.wur.nl:mobps/software.git ~/mobps-software
# Files will be at:
#   ~/mobps-software/RandomFieldsUtils_1.6.0.2.tar.gz
#   ~/mobps-software/miraculix_1.6.0.2.tar.gz
```

**Option B — Download only the tarballs** (if you don't need the full repo):

```bash
# Using curl (replace TOKEN if repo requires auth)
curl -L -o RandomFieldsUtils_1.6.0.2.tar.gz \
  https://git.wur.nl/mobps/software/-/raw/main/RandomFieldsUtils_1.6.0.2.tar.gz

curl -L -o miraculix_1.6.0.2.tar.gz \
  https://git.wur.nl/mobps/software/-/raw/main/miraculix_1.6.0.2.tar.gz
```
...
pkg_dir <- "C:/Users/yourname/mobps-software"   # adjust to your path
...
pkg_dir <- "~/mobps-software"   # adjust to your path
